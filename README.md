...
### `CleanOceanCompany`
Unity 기반 메타버스/미니게임 환경에서 손 인식 기반 상호작용과 실시간 채팅을 결합한 프로젝트 코드입니다.  
정리된 코드는 `Manager`, `Mediapipe`, `Network` 세 영역으로 나뉘며, Python Mediapipe 프로세스와 Unity 게임 로직을 연결하는 구조가 핵심입니다.

#### `Manager`
- `BrushManager.cs`
  - 브러시가 `Trash` 오브젝트와 충돌할 때 청소 횟수를 차감
  - 브러시 색상을 점점 어둡게 바꾸며 소모 상태를 시각적으로 표현
...
- `ChatManager.cs`
  ...
  - 일반 채팅과 `/w` 형식의 귓속말을 구분 처리
- `MediapipeManager.cs`
  - Unity 내부에서 Python 실행 파일과 Mediapipe 스크립트를 별도 프로세스로 실행
  - Python 표준 출력 JSON 데이터를 파싱해 브러시 위치를 갱신
  - 남은 오염물 수를 관리하고, 미니게임 종료 시 결과 처리와 프로세스 종료 수행
- `MediapipeMiniGameTimeManager.cs`
  - 제한 시간 UI, 슬라이더, 성공/실패 결과 패널 관리
  - 남은 시간 비율에 따라 포인트를 차등 지급
  - 게임 종료 시 기존 UI와 플레이어 조작 상태를 복구

#### `Mediapipe`
- `BrushTriggerEvent.cs`
  - 브러시 사용 횟수를 줄이고 RawImage 색상을 변경해 브러시 소모를 표현
- `MediapipePorpoise.cs`
  - 브러시 위치를 3D 돌고래 모델 표면에 Raycast로 투영
  - Dirt Mask 텍스처를 직접 수정해 닦이는 효과를 구현
  - 청소 진행률 계산, 파티클 생성, 완료 시 브러시 제거 및 점수 처리 연결
- `MediapipeThirdManager.cs`
  - Mediapipe JSON 입력을 큐로 받아 손 상태(`isFist`)에 따라 브러시 조작과 카메라 회전을 분기
...
- 핵심 포인트:
  ...
  - 충돌 기반 청소와 텍스처 마스킹 기반 청소 연출을 함께 구현
