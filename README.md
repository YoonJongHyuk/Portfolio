...
Unity 기반 VR 휠체어 이동/상호작용 프로젝트 코드입니다. 정리된 코드는 `Inventory`, `Player`, `WheelControl` 영역으로 나뉘며, Meta Quest 컨트롤러 입력을 바탕으로 휠체어 조작, 아이템 수집/사용, 플레이어 상태 관리, VR 시야 안정화 효과를 구현한 구조가 특징입니다.

#### `Inventory`

- `InventoryManager.cs`
  - 인벤토리 아이템 목록과 현재 무게를 관리
  - 아이템 추가/삭제 시 무게를 갱신하고 최대 무게 초과 여부에 따라 휠체어 후방 오브젝트 표시 제어
- `ItemSO.cs`
  - 아이템 이름, 설명, 타입, 무게, 스프라이트, 프리팹, 메시/재질 정보를 정의하는 ScriptableObject
  - Odin Inspector 속성을 활용해 에디터에서 아이템 데이터를 편집하기 쉽게 구성
- `Item.cs`, `ItemType.cs`
  - 실제 아이템 오브젝트와 상태 변화 대상 타입을 정의
  - 피곤함, 허기, 스테미나, 질병, 체력 등 플레이어 상태와 아이템 효과를 연결할 수 있는 구조 제공
- `ItemEffectEventTable.cs`, `ItemEventManager.cs`
  - 아이템 타입별 효과 이벤트를 연결하고 사용 시 플레이어 상태 변화로 반영
- `InventoryTrigger.cs`, `OutlineTrigger.cs`, `OnOffInventory.cs`
  - 인벤토리 진입/이탈, 외곽선 표시, 인벤토리 UI 활성화 등 상호작용 흐름 처리
- `TongsManager.cs`, `TongsTrigger.cs`
  - VR 막집게로 `Grabbable` 오브젝트를 집고 놓는 로직
  - 손 입력과 GrabInteractor 상태를 확인해 집게로 든 물체와 손으로 직접 잡은 물체의 충돌 상황을 정리
- `StandEvent.cs`, `StandItem.cs`
  - 특정 태그의 오브젝트를 스탠드 위치에 배치하거나 다시 집을 수 있도록 처리
  - 플레이어와 거리가 멀어졌을 때 배치 상태를 복구하는 로직 포함

#### `Player`

- `PlayerInputManger.cs`
  - 좌/우 컨트롤러 트리거 입력을 감지하고 최근 사용한 손을 기준으로 Raycast 상호작용 처리
  - 중복 입력 방지를 위한 쿨타임과 VR LineRenderer 기반 상호작용 표시 구조 포함
- `PlayerStatus.cs`
  - 피곤함, 허기, 스테미나, 질병, 체력 수치를 시간 흐름에 따라 갱신
  - 아이템 타입과 변화량을 받아 플레이어 상태를 증가/감소시키는 로직 제공
- `PlayerNotebook.cs`
  - 노트북 UI 탭 전환과 페이지 활성화 처리
  - 선택한 빌드 오브젝트를 카메라 전방에 생성하는 테스트/상호작용 기능 포함
- `PlayerManager.cs`
  - 플레이어 관련 참조와 상태를 통합 관리하기 위한 기본 매니저 구조

#### `WheelControl`

- `New_WheelchairManager.cs`
  - 좌/우 바퀴 입력 속도를 기반으로 휠체어 전진, 후진, 회전을 제어
  - Rigidbody에 힘과 토크를 적용하고 최대 선형/각속도를 제한
  - 양손 바퀴 조작 상태와 잘못된 회전 입력 방지를 위한 잠금 타이머 관리
  - 이동 상태에 따라 터널링 효과와 휠체어 시각 요소를 연결
- `WheelchairSpeedCheck.cs`
  - 컨트롤러가 바퀴 영역에 접촉하고 손 트리거를 누른 상태에서 바퀴 속도를 계산
  - 인덱스 트리거 입력을 브레이크로 처리하고 바퀴 모델 회전 연출을 갱신
- `ControllerSpeedCheck.cs`
  - 컨트롤러의 로컬 이동량을 휠체어 기준 전후 방향 속도로 변환
  - 좌/우 컨트롤러 타입과 GrabInteractor 참조를 기반으로 바퀴 조작 입력을 보조
- `TunnelingEffect.cs`
  - VR 이동 멀미 완화를 위한 비네트/터널링 효과 렌더링
  - 카메라 near plane, 양안 거리, 시야각, 마스크 색상 등을 기반으로 시야 제한 효과를 조정
- `WheelChairRenderer.cs`, `Wheelchair_Move_Test.cs`
  - 휠체어 렌더링 및 이동 테스트를 위한 보조 스크립트

#### 요약

- 기술 스택: `Unity`, `C#`, `Meta Quest`, `OVRInput`, `Oculus Interaction`, `Rigidbody`
- 핵심 포인트:
  - VR 컨트롤러 움직임을 휠체어 바퀴 입력으로 해석하는 이동 시스템 구현
  - 인벤토리 무게, 아이템 효과, 플레이어 상태 변화를 연결한 생존형 상호작용 구조
  - 막집게, 스탠드 배치, Raycast UI 등 VR 환경에 맞춘 오브젝트 조작 흐름 구성
  - 휠체어 이동 중 시야 안정화를 위한 터널링 효과 포함
...
- `Lines_We_Cross`: VR 휠체어 이동, 인벤토리/아이템 효과, 플레이어 상태, 막집게 상호작용 구조
